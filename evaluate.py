"""Évaluation du DQN et comparaison avec des stratégies de référence."""
...
def idle_policy(state, environment):
    """Ne charge et ne décharge jamais la batterie."""

    return EnergyEnvironment.IDLE


def rule_based_policy(state, environment):
    """Stratégie simple privilégiant l'autonomie et la batterie.

    - charge la batterie lorsqu'il existe un surplus solaire ;
    - décharge principalement pendant une coupure ;
    - conserve la batterie lorsque le réseau est disponible.
    """

    row = environment.data.iloc[environment.current_step]

    pv = float(row["pv_production"])
    consumption = float(row["consumption"])
    grid_available = int(row["grid_available"])

    if pv > consumption and environment.soc < 1.0:
...
def evaluate_policy(
    ...
    while not done:
        ...
        total_battery_charge += info["battery_charge"]
        total_battery_discharge += info["battery_discharge"]

        state = next_state

    total_consumption = float(
        environment.data["consumption"].sum()
    )

    autonomy_rate = (
        100 * (1 - total_grid_import / total_consumption)
        if total_consumption > 0
        else 0.0
    )

    demand_satisfaction_rate = (
        100 * (1 - total_unmet_demand / total_consumption)
        if total_consumption > 0
        else 0.0
    )
    ...
    outage_hours = int(
        (environment.data["grid_available"] == 0).sum()
    )

    return {
        "policy": policy_name,
        "reward": round(total_reward, 6),
        "grid_import_kwh": round(total_grid_import, 6),
        "unmet_demand_kwh": round(total_unmet_demand, 6),
        "battery_charge_kwh": round(total_battery_charge, 6),
        ...
        "autonomy_rate_percent": round(autonomy_rate, 4),
        "demand_satisfaction_percent": round(
            demand_satisfaction_rate,
            4,
        ),
        "final_soc": round(environment.soc, 6),
        "outage_hours": outage_hours,
        "idle_actions": action_counts[EnergyEnvironment.IDLE],
        "charge_actions": action_counts[EnergyEnvironment.CHARGE],
...
def main() -> None:
    ...
    complete_data = load_environment_data(seed=42)
    _, _, test_data = split_environment_data(complete_data)

    idle_results = evaluate_policy(
        policy_name="idle",
        environment=EnergyEnvironment(test_data),
    ...
    dqn_results = evaluate_policy(
        policy_name="dqn",
        environment=EnergyEnvironment(test_data),
        action_selector=lambda state, environment: agent.act(
            state,
            explore=False,
        ),
    )
    ...
    for result in results:
        print(
            f"{result['policy']:12} | "
            f"récompense : {result['reward']:9.2f} | "
            f"import : {result['grid_import_kwh']:8.2f} kWh | "
            f"non satisfaite : "
    ...
    print("=" * 74)
    print("Résultats enregistrés dans :", RESULTS_PATH)
